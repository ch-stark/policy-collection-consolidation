...
**Source:** [open-cluster-management-io/policy-collection#553](https://github.com/open-cluster-management-io/policy-collection/pull/553)

**Name:** Operator Guardrails for OpenShift  
**Type:** RHACM PolicySet (community)  
**Purpose:** Centralized operator compliance audit and admission warnings  
**Path in this repo:** `policies/policy-sets/community/operatorguardrails/`
...
```
policies/policy-sets/community/operatorguardrails/
├── README.md
├── kustomization.yml
├── policyGenerator.yaml
├── placement.yaml
└── input/auditoperators/
    ├── auditpolicy.yaml      # Inform: audit existing Subscriptions
    ├── configmaps.yaml       # Allowlist/denylist on hub
    └── vap.yaml              # ValidatingAdmissionPolicy (Warn)
```
...
```bash
cd policies/policy-sets/community/operatorguardrails
kustomize build --enable-alpha-plugins
```

Deploy as a separate ArgoCD Application or add to `environments/<env>/kustomization.yaml`.
...
**Key Elements**:
- Policy Namespace: `acm-policies` (overridden per environment via kustomize)
